...
define_flashy_header <- function(title, subtitle, header_color,
  ...
  grid::grid.text(title, vp = vplayout(1, 1:3), just = "left",
                  x = 0.05, y = 0.65, gp = grid::gpar(fontsize = 22, col = "white",
                                                fontface = "bold"))
...
set_flashy_report_page_theme <- function(ggplot_object, header_color,
  ...
  set_layout(7, 3,
             ...
             c(2, 6, 2))
  if (need_header == TRUE) {
    define_flashy_header(myprettyreport.env$report_title, myprettyreport.env$report_subtitle,
                         header_color, footer_color)
    from <- 2
  }
  if (need_footer == TRUE) {
  ...
  print(ggplot_object, vp = vplayout(from:to, 1:3))
}
...
set_flashy_cover_theme <- function(header_color, footer_color,
  ...
  grid::grid.text(myprettyreport.env$report_title, vp = vplayout(7:8, 3:4),
                  x = 0.05, y = 0.7, just = "left",
                  gp = grid::gpar(fontsize = 32, fontface = "bold"))
  grid::grid.text(myprettyreport.env$report_subtitle, vp = vplayout(9, 2:4),
